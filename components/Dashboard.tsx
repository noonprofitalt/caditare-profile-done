...
import { WorkTask, SystemAlert } from '../types';
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTasks(TaskEngine.generateWorkQueue(MOCK_CANDIDATES));
    setAlerts(TaskEngine.generateAlerts(MOCK_CANDIDATES));
  }, []);
  ...
  const activeCandidates = MOCK_CANDIDATES.length;
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6 relative h-[calc(100vh-4rem)] overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between group hover:border-red-200 transition-colors">
           <div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between">
           <div>
              <p className="text-slate-500 text-xs uppercase font-bold tracking-wider mb-1">Tasks Due Today</p>
              <h3 className="text-3xl font-bold text-slate-800">{tasks.filter(t => t.dueDate === 'Today').length}</h3>
              <p className="text-xs text-slate-400 mt-1">Personal Work Queue</p>
           </div>
           <div className="bg-blue-50 p-3 rounded-lg"><CheckCircle size={24} className="text-blue-600" /></div>
        </div>
         ...
         <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between">
           <div>
              ...
              <p className="text-xs text-green-600 mt-1 font-medium flex items-center"><TrendingUp size={10} className="mr-1" /> +3 this week</p>
           </div>
              ...
              <div className="space-y-3">
                 {alerts.length > 0 ? alerts.map(alert => (
                    <div key={alert.id} className={`p-3 rounded-lg text-sm border ${
                       alert.type === 'DELAY' ? 'bg-red-50 border-red-100 text-red-800' : 
         ...
         <div className="group relative">
            ...
            <div className="absolute bottom-full right-0 mb-3 w-48 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-bottom-right scale-95 group-hover:scale-100">
               <div className="p-2 space-y-1">
                  ...
                  <button className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg text-left">
