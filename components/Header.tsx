...
import { Bell, Search, UserCircle, X, ChevronRight, Command, MessageSquare } from 'lucide-react';
import { MOCK_CANDIDATES } from '../services/mockData';
import { Candidate, WorkflowStage, Employer, Job } from '../types';
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for storage events (e.g. from service)
  ...
  // Helper to safely get candidates
  const Method_GetCandidates = () => {
    try {
      return CandidateService.getCandidates() || [];
    } catch (e) {
      return MOCK_CANDIDATES; // Fallback
    }
  };
  ...
  const handleSelect = (type: 'candidate' | 'employer' | 'job', id: string) => {
    ...
    if (type === 'job') navigate(`/jobs?id=${id}`); // Or handle job detail view logic
    setQuery('');
    setIsOpen(false);
  };
    ...
    <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-40 px-6 flex items-center justify-between shadow-sm">
      ...
      <div className="relative w-full max-w-xl" ref={wrapperRef}>
        ...
        {isOpen && (results.candidates.length > 0 || results.employers.length > 0 || results.jobs.length > 0) && (
          <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-100 origin-top max-h-[80vh] overflow-y-auto">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 overflow-hidden ring-2 ring-transparent hover:ring-blue-500 transition-all">
              {user?.avatar ? (
      ...
      {isChatOpen && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-[400px] bg-white shadow-2xl border-l border-slate-200 flex flex-col animate-in slide-in-from-right duration-500" style={{ zIndex: 9999 }}>
